<?php
// *************************************************************************
//  This file is part of ZR-Zombie-Rank sourcemod plugin.
//  https://github.com/hallucinogenic/-ZR-Zombie-Rank
//
//  Copyright (C) 2017 Thomas 'shanapu' Schmidt
//
...
<html data-ng-app="app">
	<head>
		...
		<title><?php echo "$siteTitle"?> - ZRank</title>
	...
	if ($background != "")
	{
		echo'<body data-ng-controller="demoController as vm" style="background: url(' .$background. '); background-color: #1b2838; background-repeat: no-repeat; background-attachment: fixed; background-size:100%;">';
	}
	else
	{
	...
	?>
		...
		<main class="container">
			...
			<?php
				...
				{
					echo '<div class="alert alert-danger" role="alert"><strong>Failed to load php extensions GMP</strong></div>';
				}
			?>
				...
				<?php
					...
					// SQL class
					$sql_zombieranks = new SQL($zombieranks_dbHost, $zombieranks_dbUser, $zombieranks_dbPass, $zombieranks_dbName);
					...
					// WHERE clause
					$sqlSearch = "WHERE `points` >= $zombieranks_minPoints";
					...
					{
						// Escape Search
						$search = $sql_zombieranks->escape($search);
						...
						if ($searchTyp == "SteamID")
						{
							...
							$steamid3 = $steamid->RenderSteam3().PHP_EOL;
					...
					$pointsTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=points&amp;sort=desc"><strong>Points</strong></a>';
					$infTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=infects&amp;sort=desc"><strong>Humans infect</strong></a>';
					$zkTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=z_kills&amp;sort=desc"><strong>Zombie kills</strong></a>';
					$zwTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=z_wins&amp;sort=desc"><strong>Zombie wins</strong></a>';
					$hwTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=h_wins&amp;sort=desc"><strong>Human wins</strong></a>';
					...
					if (isset($_GET["type"]) && isset($_GET["sort"]))
					{
						...
						else if ($_GET["type"] == "points")
						{
							...
							$pointsTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=points&amp;sort=' .$op. '"><strong>Points</strong></a>' .$sortImg;
							$site .= "type=points&amp;";
						}
						else if ($_GET["type"] == "infects")
						{
							$sqlSearch .= " ORDER by `human_infects`";
							$infTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=infects&amp;sort=' .$op. '"><strong>Humans infect</strong></a>' .$sortImg;
							$site .= "type=infects&amp;";
						}
						else if ($_GET["type"] == "z_kills")
						{
							$sqlSearch .= " ORDER by `zombie_kills`";
							$zkTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=z_kills&amp;sort=' .$op. '"><strong>Zombie kills</strong></a>' .$sortImg;
							$site .= "type=z_kills&amp;";
						}
						else if ($_GET["type"] == "z_wins")
						{
							$sqlSearch .= " ORDER by `roundwins_zombie`";
							$zwTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=z_wins&amp;sort=' .$op. '"><strong>Zombie wins</strong></a>' .$sortImg;
							$site .= "type=z_wins&amp;";
						}
						else if ($_GET["type"] == "h_wins")
						{
							$sqlSearch .= " ORDER by `roundwins_human`";
							$hwTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=h_wins&amp;sort=' .$op. '"><strong>Human wins</strong></a>' .$sortImg;
							$site .= "type=h_wins&amp;";
						}
						else
						{
					...
					else
					{
						...
						$pointsTable = '<a href="index.php' .$site. 'page=' .$currentSite. '&amp;type=points&amp;sort=asc"><strong>Points</strong></a> <span class="caret"></span> ';
					}
					
					// Calculate Entrys
					$totalEntrys = $sql_zombieranks->getRows($sql_zombieranks->query("SELECT COUNT(`playername`) FROM `zrank` $sqlSearch"));
					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							<div class="table-responsive"><table class="table table-striped table-hover table-outside-bordered" data-link="row">
								<?php
									// Get entrys
									$result = $sql_zombieranks->query("SELECT * FROM `zrank` $sqlSearch LIMIT $firstItem, $usersPerPage");
									
									// Have any entrys?
									if ($sql_zombieranks->foundData($result))
									{
										...
										<th style="width: 25%; padding-left: 3px; ">' .$nameTable. '</th>
										<th style="width: 12%; padding-left: 3px; ">' .$pointsTable. '</th>
										<th style="width: 12%; padding-left: 3px; ">'. $infTable. '</th>
										<th style="width: 12%; padding-left: 3px; ">' .$zkTable. '</th>
										<th style="width: 12%; padding-left: 3px; ">' .$zwTable. '</th>
										<th style="width: 12%; padding-left: 3px; ">' .$hwTable. '</th>
										<th style="width: 2%; "></th></thead>';
										
										// Loop through query
										while ($row = $sql_zombieranks->getArray($result))
										{
											...
											$steam2 = $steamid->RenderSteam2().PHP_EOL;
											$steam64 = $steamid->ConvertToUInt64().PHP_EOL;
											$steam2 = preg_replace('/\s+/', '', $steam2);
											$steam64 = preg_replace('/\s+/', '', $steam64);
											
											if(($search == $steam2 || $search == $steam64 || $search == $row['SteamID'] || $search == $name) && $search != "")
											{
											...
											<td>' .$row['points']. '</td>
											<td>' .$row['human_infects']. '</td>
											<td>' .$row['zombie_kills']. '</td>
											<td>' .$row['roundwins_zombie']. '</td>
											<td>' .$row['roundwins_human']. '</td>
